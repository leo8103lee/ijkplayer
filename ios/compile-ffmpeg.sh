...
FF_ALL_ARCHS_IOS8_SDK="armv7 arm64 i386 x86_64 arm64-sim"
...
if [ "$FF_TARGET" = "armv7" -o "$FF_TARGET" = "armv7s" -o "$FF_TARGET" = "arm64" ]; then
    ...
    do_lipo_all
elif [ "$FF_TARGET" = "i386" -o "$FF_TARGET" = "x86_64" -o "$FF_TARGET" = "arm64-sim" ]; then
    echo_archs
    sh tools/do-compile-ffmpeg.sh $FF_TARGET $FF_TARGET_EXTRA
    do_lipo_all
    ...
    echo "Usage:"
    echo "  compile-ffmpeg.sh armv7|arm64|i386|x86_64|arm64-sim"
